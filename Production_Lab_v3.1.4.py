...
def get_video_info(video_path):
    ...
    # Get detailed codec/audio info via ffprobe
    try:
        cmd = [
        ...
        if result.returncode == 0:
            ...
            for stream in probe_data.get("streams", []):
                if stream.get("codec_type") == "video":
                    ...
                    if "bit_rate" in stream:
                        info["video_bitrate"] = int(stream["bit_rate"]) // 1000  # kbps
            ...
            if "bit_rate" in fmt:
                info["bitrate"] = int(fmt["bit_rate"]) // 1000  # kbps
    except Exception:
        pass  # Fall back to basic info if ffprobe fails
...
def update_token_estimate(dur, fps, w, h, limit, media_type='video', max_model_len=None):
    """Calculates estimated visual tokens vs context limit"""
    # Use max_model_len if available, otherwise fall back to config limit
    context_limit = max_model_len if max_model_len else limit
    ...
    if media_type == 'image':
        ...
        remaining = context_limit - total_visual_tokens
        ...
        status += f"- **Thinking Buffer:** <span style='color:{color}'>{int(remaining):,}</span> / {context_limit:,}\n"
        status += f"- **Context Limit:** {context_limit:,} ({limit_source})"
        return status
...
def refresh_models(api_url):
    """Fetch available models and max_model_len from vLLM"""
    try:
        ...
        if response.status_code == 200:
            ...
            # Try to get max_model_len from first model
            max_model_len = None
            if data:
                max_model_len = data[0].get('max_model_len')
            return gr.update(choices=models, value=models[0] if models else ""), max_model_len
    except: pass
    return gr.update(choices=["Offline / Check URL"]), None
...
def chat_streaming(media_path, user_message, chat_history, api_history, last_media_path, api_url, model_name, processing_mode, 
                   ...
                   max_tokens, temperature, top_p, min_p, top_k, rep_p, pres_p, freq_p, seed,
...
def build_interface():
    ...
    with gr.Blocks(theme=theme, css=custom_css, title="Vision Lab - VLM Interface") as app:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("⚙️ Inference Tuning", open=False):
                    ...
                    with gr.Row():
                        seed_input = gr.Number(label="Seed", value=config.get("seed", -1), precision=0,
                                              info="-1 = random each request")
                        randomize_seed_btn = gr.Button("🎲", scale=0, min_width=50)
            ...
            with gr.Column(scale=2):
                ...
                max_model_len_state = gr.State(None)  # Track max_model_len from vLLM
        ...
        # Logic
        def on_media_load(file_obj, curr_target_fps, curr_res_mode, curr_img_w, curr_img_h, curr_max_model_len):
            if not file_obj:
            ...
            if media_type == 'image':
                ...
                token_md = update_token_estimate(0, 0, eff_w, eff_h, config['vram_limit'], 'image', curr_max_model_len)
                ...
                if curr_res_mode == "Native Resolution":
                    ...
                    eff_w, eff_h = curr_img_w or 640, curr_img_h or 480
                token_md = update_token_estimate(info.get('dur', 0), curr_target_fps, eff_w, eff_h, config['vram_limit'], 'video', curr_max_model_len)
                
                # Build detailed video info display
                type_md = f"### Media Type: 🎬 Video\n"
                type_md += f"**Resolution:** {info.get('res', 'Unknown')}\n"
                type_md += f"**Duration:** {info.get('dur', 0):.1f}s | **FPS:** {info.get('fps', 0):.1f}\n"
                type_md += f"**Video Codec:** {info.get('video_codec', 'Unknown')}"
                if info.get('bitrate'):
                    type_md += f" @ {info.get('bitrate')}kbps"
                type_md += "\n"
                
                # Audio info
                if info.get('audio_codec') and info.get('audio_codec') != "None":
                    audio_str = f"**Audio:** {info.get('audio_codec')}"
                    if info.get('audio_channels'):
                        ch = info.get('audio_channels')
                        ch_label = "Mono" if ch == 1 else "Stereo" if ch == 2 else f"{ch}ch"
                        audio_str += f" ({ch_label})"
                    if info.get('audio_sample_rate'):
                        audio_str += f" @ {info.get('audio_sample_rate')//1000}kHz"
                    type_md += audio_str
                else:
                    type_md += "**Audio:** None"
        ...
        raw_media.change(on_media_load, inputs=[raw_media, target_fps, resolution_mode, image_width, image_height, max_model_len_state], 
                        outputs=[start_time, end_time, token_status, media_type_display, media_preview, analysis_media, media_info_state])
        
        def recalc_tokens(media_info, curr_target_fps, curr_res_mode, curr_img_w, curr_img_h, curr_max_model_len):
            ...
            if media_type == 'image':
                return update_token_estimate(0, 0, eff_w, eff_h, config['vram_limit'], 'image', curr_max_model_len)
            elif media_type == 'video':
                dur = media_info.get('dur', 0)
                return update_token_estimate(dur, curr_target_fps, eff_w, eff_h, config['vram_limit'], 'video', curr_max_model_len)
            return "### Token Status\nUpload media to estimate..."
        
        # Wire up dynamic token recalculation
        token_recalc_inputs = [media_info_state, target_fps, resolution_mode, image_width, image_height, max_model_len_state]
        target_fps.change(recalc_tokens, inputs=token_recalc_inputs, outputs=token_status)
        resolution_mode.change(recalc_tokens, inputs=token_recalc_inputs, outputs=token_status)
        image_width.change(recalc_tokens, inputs=token_recalc_inputs, outputs=token_status)
        image_height.change(recalc_tokens, inputs=token_recalc_inputs, outputs=token_status)
        max_model_len_state.change(recalc_tokens, inputs=token_recalc_inputs, outputs=token_status)
        ...
        refresh_btn.click(refresh_models, api_url, [model_name, max_model_len_state])
        
        def randomize_seed():
            return random.randint(0, 2**31 - 1)
        randomize_seed_btn.click(randomize_seed, outputs=seed_input)
        ...
        def run_chat(analysis_file, user_msg, chat_hist, api_hist, last_media, api_url, model_name, proc_mode, 
                     ...
                     top_p, min_p, top_k, rep_p, pres_p, freq_p, seed, thought_syntax, interaction_mode, 
        ...
        run_event = run_btn.click(
            ...
            inputs=[analysis_media, prompt, chatbot, api_history_state, last_media_state, api_url, model_name, 
                    ...
                    seed_input, thought_syntax, interaction_mode, system_prompt, inject_thinking, uncensored_mode, max_images], 
        ...
        save_btn.click(
            fn=lambda *args: save_config({
                ...
                "frequency_penalty": args[15], "seed": args[16], "thought_syntax": args[17], "interval": args[18],
                "interaction_mode": args[19], "system_prompt": args[20], "inject_thinking_tags": args[21],
                "uncensored_mode": args[22], "max_images_in_context": args[23]
            }),
            inputs=[api_url, model_name, proc_mode, samp_mode, target_fps, image_width, image_height, resolution_mode, prompt, max_t, temp, top_p, min_p, rep_p, pres_p, freq_p, seed_input, thought_syntax, interval_in, interaction_mode, system_prompt, inject_thinking, uncensored_mode, max_images],
