...
def make_markdown_instruction(thought_syntax):
    ...
    return f"""### SYSTEM INSTRUCTIONS ###
     - Analyze the video and provide the requested marketing clips.
...
def build_interface():
    ...
    with gr.Blocks(theme=theme, title="Promo Sifter Production Lab") as app:
        ...
        def on_video_load(path):
            ...
            token_md = update_token_estimate(info['dur'], config['target_fps'], config['image_width'], config['image_height'], config['vram_limit'])
        ...
        run_event = run_btn.click(process_video_streaming, 
            [analysis_video, api_url, model_name, proc_mode, prompt, samp_mode, interval_in, target_fps, gr.State(0), proc_w, proc_h, max_t, temp, top_p, min_p, gr.State(20), rep_p, gr.State(0), freq_p, thought_syntax], 
        ...
        save_btn.click(
            fn=lambda *args: save_config({
                ...
                "repetition_penalty": args[12], "frequency_penalty": args[13], 
                "thought_syntax": args[14], "interval": args[15]
            }),
            inputs=[api_url, model_name, proc_mode, samp_mode, target_fps, img_w, img_h, prompt, max_t, temp, top_p, min_p, rep_p, freq_p, thought_syntax, interval_in],
